...
assert.equal(extractNumber(siteJs, 'flowerPatternRotationDegrees'), 30);
assert.equal(extractNumber(siteJs, 'flowerGridRadius'), 4);
assert.match(siteJs, /function getTokenAngle\(index\)/);
assert.match(siteJs, /function createSvgArc\(/);
...
assert.match(siteJs, /clipPath\.setAttribute\('id', 'flowerPatternClip'\)/);
assert.doesNotMatch(siteJs, /createElementNS\('http:\/\/www\.w3\.org\/2000\/svg', 'polygon'\)/);
assert.doesNotMatch(siteJs, /flowerHexClipRadiusRatio/);
assert.match(siteJs, /clippedPattern\.setAttribute\('clip-path', 'url\(#flowerPatternClip\)'\)/);
assert.match(siteJs, /outerRingRadius\s*=\s*circleRadius\s*\*\s*3/);
assert.match(siteJs, /rotationRadians\s*=\s*flowerPatternRotationDegrees\s*\*\s*Math\.PI\s*\/\s*180/);
assert.match(siteJs, /gridX \* rotationCos - gridY \* rotationSin/);
assert.match(siteJs, /distanceFromCenter\s*>=\s*outerRingRadius\s*\+\s*circleRadius/);
assert.match(siteJs, /intersections\.forEach\(\(point\) =>/);
assert.match(siteJs, /uniqueAngles\.forEach\(\(startAngle, index\) =>/);
assert.match(siteJs, /createSvgArc\('flower-circle'/);
